(ns foodmachmfg.registry-test
  (:require [clojure.test :refer [deftest is]]
            [foodmachmfg.registry :as r]))
...
(deftest known-product-types-are-valid
  (doseq [pt [:mixing-machine :filling-machine :packaging-machine
...
(deftest typical-no-load-run-speed-is-valid
  ...
  (is (r/no-load-run-speed-rpm-valid? 120.0))
  (is (r/no-load-run-speed-rpm-valid? 900.0))
  (is (r/no-load-run-speed-rpm-valid? 3000.0)))
...
(deftest excessive-no-load-run-speed-is-invalid
  ...
  (is (not (r/no-load-run-speed-rpm-valid? 3000.01))))
...
(deftest history-is-append-only
  (let [c1 (r/register-maintenance "mnt-1" "mix-001" 0)
        hist (r/append [] c1)
        c2 (r/register-maintenance "mnt-2" "mix-001" 1)
