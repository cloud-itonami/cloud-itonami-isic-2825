(ns foodmachmfg.governor-contract-test
  ...
  operations... does NOT self-issue a machinery or food-contact-
  material compliance certification mark') implemented faithfully. The
  single invariant under test:

    FoodMachAdvisor never schedules maintenance, flags a safety
    concern, or coordinates a shipment the Food, Beverage and Tobacco
  ...
  (let [[db actor] (fresh)
        res (exec-op actor "t1"
                  {:op :log-production-batch :effect :propose :subject "batch-001"
                   :patch {:product-type :mixing-machine}} coordinator)]
    (is (= :commit (get-in res [:state :disposition])))
    (is (= :mixing-machine (:product-type (store/batch db "batch-001"))) "SSoT actually updated")
...
(deftest certification-authority-is-held-and-permanently-blocked
  (testing "a proposal that sets :issue-certification? true -> HOLD, PERMANENT, never reaches request-approval -- this actor is never the machinery/food-contact-material compliance certification authority"
    (let [[db actor] (fresh)
    ...
    (let [[db actor] (fresh)
          res (exec-op actor "t12" {:op :flag-safety-concern :effect :propose :subject "concern-1"
                                    :value {:equipment-id "mix-001" :severity :moderate
                                            :description "mixing-blade vibration anomaly"}}
                       coordinator)]
      (is (= :interrupted (:status res)))
...
(deftest safety-concern-approval-rejected-leaves-no-record-only-a-hold-fact
  (let [[db actor] (fresh)
        _ (exec-op actor "t13" {:op :flag-safety-concern :effect :propose :subject "concern-2"
                                :value {:equipment-id "mix-001" :severity :low :description "y"}}
                   coordinator)
        r (reject! actor "t13")]
