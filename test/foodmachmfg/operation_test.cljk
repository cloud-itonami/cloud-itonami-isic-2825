(ns foodmachmfg.operation-test
  "Smoke tests for the compiled FoodMachOperationActor graph itself
  (build + one happy path per op). The governor's full rule contract
  (HARD holds, escalation, phase gating) is exercised in
  `foodmachmfg.governor-contract-test`; the Store contract in
  `foodmachmfg.store-contract-test`."
...
(deftest test-actor-builds
  (testing "FoodMachOperationActor can be built with a store"
    ...
    (let [s (-> (store/mem-store) (store/sample-data!))
          ...
          result (exec-op actor "t3"
                          {:op :flag-safety-concern :effect :propose :subject "concern-1"
                           :value {:equipment-id "mix-001" :severity :moderate :description "mixing-blade vibration anomaly"}}
                          coordinator)]
      (is (= :interrupted (:status result))))))
