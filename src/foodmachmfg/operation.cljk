(ns foodmachmfg.operation
  "FoodMachOperationActor -- one plant-operations coordination request
  = one supervised actor run, expressed as a langgraph-clj StateGraph.
  The advisor (FoodMachAdvisor) is sealed into a single node (:advise);
  its proposal is ALWAYS routed through the Food, Beverage and Tobacco
  ...
  (:require [langgraph.graph :as g]
            ...
            [foodmachmfg.advisor :as advisor]
...
(defn build
  "Compiles a FoodMachOperationActor graph bound to `store`.
  opts:
    :advisor      -- a `foodmachmfg.advisor/Advisor` (default: mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; FoodMachAdvisor inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (advisor/-advise advisor store request)]
            {:proposal p :audit [(advisor/trace request p)]})))

      ;; Food, Beverage and Tobacco Machinery Plant Operations Governor -- independent censor.
      (g/add-node :govern
