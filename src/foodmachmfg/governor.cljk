(ns foodmachmfg.governor
  "Food, Beverage and Tobacco Machinery Plant Operations Governor --
  the independent compliance layer that earns the FoodMachAdvisor the
  ...
  secretly tries to self-issue a machinery safety or food-contact-
  material compliance certification mark (an authority this actor
  ...
  assembly/test-bench-equipment control, so this MUST be a separate
  system able to *reject* a proposal and fall back to HOLD.

  `:itonami.blueprint/governor` is
  `:food-beverage-tobacco-machinery-plant-operations-governor` (see
  ...
  gate (SOFT -- asks a human to look, and the human may approve):
    ...
    5. Certification-authority
       blocked                     -- ANY proposal (any op) whose own
                                       ...
                                       certification mark (e.g. CE
                                       marking under the EU Machinery
                                       Regulation, or food-contact
                                       compliance under EU Regulation
                                       1935/2004 / FDA 21 CFR 178 /
                                       NSF/ANSI 169 hygienic-design
                                       standards) -- an authority
  ...
  (when-not (contains? allowed-proposal-effects (:effect proposal))
    [{:rule :equipment-control-blocked
      :detail (str "proposal :effect (" (pr-str (:effect proposal))
                   ") はミキサー・充填機・包装機等の試験設備の直接操作に該当する可能性があり、恒久的に禁止")}]))
...
(defn- equipment-actuate-blocked-violations
  ...
  (when (and (= op :schedule-maintenance)
    ...
    [{:rule :equipment-actuate-blocked
      :detail "ミキサー・充填機・包装機等の試験設備の直接操作(actuate)提案は恒久的に禁止 -- 提案(draft)のみ許可"}]))
...
(defn- certification-authority-blocked-violations
  ...
  to self-issue a machinery safety or food-contact-material compliance
  certification mark (e.g. CE marking under the EU Machinery
  Regulation, or food-contact compliance under EU Regulation
  1935/2004 / FDA 21 CFR 178 / NSF/ANSI 169 hygienic-design standards)
  ...
  (let [payload (or (:value proposal) (:patch proposal))]
    (when (true? (:issue-certification? payload))
      [{:rule :certification-authority-blocked
        :detail "機械安全認証(CE等)または食品接触材料適合認証の自己発行提案は恒久的に禁止 -- 認証機関の専権事項"}])))
...
(defn check
  "Censors a FoodMachAdvisor proposal against the governor rules.
