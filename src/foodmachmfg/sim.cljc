(ns foodmachmfg.sim
  ...
  proposal that tries to self-issue a machinery/food-contact-material
  compliance certification mark (permanently blocked, no override), a
  ...
  an implausible no-load-run-speed reading, and a production-batch
...
(defn -main [& _args]
  (let [db (-> (store/mem-store) (store/sample-data!))
    ...
    (println "== schedule-maintenance mnt-1 on mix-001 (verified, registered, mixing assembly line -- escalates, approve) ==")
    ...
    (println "== flag-safety-concern concern-1 on mix-001 (always escalates -- approve) ==")
    ...
    (println "== schedule-maintenance mnt-2 on pack-002 (UNVERIFIED/unregistered packaging test bench -> HARD hold) ==")
    ...
    (println "== schedule-maintenance mnt-3 on mix-001 with :actuate-equipment? true -> HARD hold, PERMANENT, never reaches a human ==")
    ...
    (println "== log-production-batch batch-001 attempting to self-issue a machinery/food-contact-material compliance certification -> HARD hold, PERMANENT ==")
