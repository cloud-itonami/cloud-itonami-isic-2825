(ns foodmachmfg.store
  "SSoT for the food, beverage and tobacco processing-machinery plant-
  ...
  is self-contained (no external food/beverage/tobacco-machinery-
  ...
  Four kinds of entity live here:
    ...
    - `equipment`         -- a mixing-equipment/packaging-equipment
                             assembly-or-test-bench unit's own record.
    ...
    - `maintenance`       -- a scheduled assembly/test-bench-equipment
                             ...
                             of equipment (`foodmachmfg.registry`'s
    ...
    - `shipments`         -- a proposed outbound food/beverage/tobacco
                             processing-machinery shipment DRAFT
                             (`foodmachmfg.registry`'s
                             `register-shipment`).
  ...
  coordinator needs."
  (:require [foodmachmfg.registry :as registry]))
...
(defprotocol Store
  ...
  (ledger [s])
  (maintenance-history [s] "the append-only maintenance-schedule history (foodmachmfg.registry drafts)")
  (shipment-history [s] "the append-only shipment-coordination history (foodmachmfg.registry drafts)")
...
(defn- sample-batches []
  {"batch-001" {:id "batch-001" :product-type :mixing-machine
                :model "MIX-500L" :no-load-run-speed-rpm 120.0
                :quantity-units 1000.0 :defect-rate-percent 1.2
                :verified? true :registered? true
                :shipped-units 200.0
                :last-assessed "2026-06-01"}
   "batch-002" {:id "batch-002" :product-type :filling-machine
                :model "FIL-3000" :no-load-run-speed-rpm 600.0
                :quantity-units 300.0 :defect-rate-percent 0.9
                :verified? true :registered? true
                :shipped-units 280.0
                :last-assessed "2026-06-01"}
   "batch-003" {:id "batch-003" :product-type :packaging-machine
                :model "PKG-1200" :no-load-run-speed-rpm 900.0
...
(defn- sample-equipment []
  {"mix-001" {:id "mix-001" :kind :mixing-equipment-assembly-line
              :verified? true :registered? true
              :last-maintenance-date "2026-05-01"}
   "pack-002" {:id "pack-002" :kind :packaging-equipment-test-bench
               :verified? false :registered? false
...
(defn- schedule-maintenance!
  ...
  maintenance-schedule record via `foodmachmfg.registry` and returns
...
(defn- propose-shipment!
  ...
  shipment-coordination record via `foodmachmfg.registry` and returns
  {:result .. :patch ..} for the caller to persist."
  [s shipment-id]
...
(defn sample-data!
  ...
  mixing-equipment-assembly-line unit (schedulable for maintenance),
  one UNVERIFIED/unregistered packaging-equipment-test-bench unit
