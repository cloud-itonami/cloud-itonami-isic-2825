(ns foodmachmfg.advisor
  "FoodMachAdvisor -- the *contained intelligence node* for the food,
  beverage and tobacco processing-machinery (industrial mixers,
  filling machines, packaging machines, bottling lines and tobacco
  processing machines) plant-operations coordination actor.
  ...
  a safety-concern flag, and drafts an outbound food/beverage/tobacco
  processing-machinery shipment coordination proposal against a
  ...
  actuation, freight dispatch, or machinery/food-contact-material
  compliance certification -- see README `What this actor does NOT
  do`. Every output is censored downstream by `foodmachmfg.governor`
  ...
  Proposal shape (all kinds):
    {:summary    str            ; human-facing draft / finding
     :rationale  str            ; why -- informational only, NOT trusted
                                 ...
                                 ; check (see `foodmachmfg.governor`)
  ...
  `foodmachmfg.governor` HARD-holds any request that doesn't, so a
  mis-wired caller can never reach a commit path even if this advisor
  were compromised."
  (:require #?(:clj  [clojure.edn :as edn]
               :cljs [cljs.reader :as edn])
            [kotoba.lang.text :as str]
            [foodmachmfg.registry :as registry]
...
(defn- schedule-maintenance
  ...
  `foodmachmfg.governor` NEVER trusts this report -- it independently
...
(defn- flag-safety-concern
  "Draft a mechanical-safety/food-contact-material-compliance concern.
  ...
  technicality). See `foodmachmfg.phase`: no phase ever adds this op
  to a phase's `:auto` set; `foodmachmfg.governor` also always
...
(defn- coordinate-shipment
  "Draft an outbound food/beverage/tobacco processing-machinery
  ...
  does NOT invent one, and `foodmachmfg.governor` NEVER trusts it: it
...
(def ^:private system-prompt
  (str "あなたは食品・飲料・たばこ加工用機械(ミキサー・充填機・"
       "包装機・ボトリングライン・たばこ加工機等の食品/飲料/たばこ"
       "加工用機械)製造工場プラント運用コーディネーターの助言者です。"
       ...
       "ミキサー・充填機・包装機等の設備の直接操作(actuate)を絶対に提案してはいけません"
       "(この actor は提案のみを行い、実行は一切行いません)。"
       "機械安全認証(CE等)や食品接触材料適合認証を自己発行する提案をしてはいけません。"
       "出荷数量を偽って報告してはいけません。"))
...
(defn- parse-proposal
  ...
  yields a safe low-confidence noop so `foodmachmfg.governor`
...
(defn trace
  ...
  [request proposal]
  {:t          :foodmachmfg-advisor-proposal
   :op         (:op request)
