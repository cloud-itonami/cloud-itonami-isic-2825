(ns foodmachmfg.registry
  "Pure-function domain logic for the food, beverage and tobacco
  processing-machinery plant-operations coordination actor --
  ...
  pre-existing `kotoba-lang/foodmachmfg`-style capability library to
  ...
  of issuing a machinery or food-contact-material compliance
  certification mark (this actor NEVER does any of those -- see
  README `What this actor does NOT do`).

  SCOPE: ISIC 2825 covers manufacture of machinery for food, beverage
  and tobacco processing -- mixing-equipment and packaging-equipment
  assembly and testing lines producing finished industrial mixers,
  filling machines, packaging machines, bottling lines and tobacco
  processing machines. This actor coordinates the back-office
  record-keeping around that plant (production-batch logging,
  maintenance scheduling, safety-concern flagging, shipment
  coordination) -- it never touches the assembly/test-bench equipment
  directly, and it never stands in for the certification body that
  issues machinery safety or food-contact-material compliance marks
  (e.g. CE marking under the EU Machinery Regulation, or food-contact
  compliance under EU Regulation 1935/2004 / FDA 21 CFR 178 / NSF/ANSI
  169 hygienic-design standards).")
...
(def valid-product-types
  ...
  through."
  #{:mixing-machine :filling-machine :packaging-machine
    :bottling-machine :tobacco-processing-machine})
...
(def no-load-run-speed-rpm-max
  ...
  across this vertical's own equipment classes (industrial food mixer
  drive shafts approximately 20-1,500 rpm, packaging-machine
  indexing/conveyor drive motors typically under 2,000 rpm,
  filling-machine nozzle-head indexing drives and bottling-line
  conveyor drives typically under 3,000 rpm) and by the general
  mechanical-safety framework the EU Machinery Regulation (EU)
  2023/1230 establishes for this equipment class -- this ceiling
  generously bounds well above any real routine test on any
  standardized machine in this vertical; a reading above it is
  implausible sensor/QC data, not a real test."
  3000.0)
...
(defn- unsigned-certificate
  ...
  actor's. And NEVER a machinery safety or food-contact-material
  compliance certification mark -- this actor is never the
...
(defn register-maintenance
  ...
  the RECORD a plant coordinator would keep. `foodmachmfg.governor`
...
(defn register-shipment
  ...
  outbound food/beverage/tobacco processing-machinery shipment against
  ...
  coordinator would keep. `foodmachmfg.governor` independently
